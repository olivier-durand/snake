...
# Fonction pour obtenir la direction optimale pour atteindre la pomme
def get_best_direction():
    # Obtenir la position de la tête du serpent
    head = snake_segments[0].topleft

    # Obtenir la position de la pomme
    apple_position = apple.topleft

    # Calculer les distances entre la tête du serpent et la pomme dans les quatre directions possibles
    distances = [
        math.sqrt((head[0] + CELL_SIZE - apple_position[0])**2 + (head[1] - apple_position[1])**2),
        math.sqrt((head[0] - CELL_SIZE - apple_position[0])**2 + (head[1] - apple_position[1])**2),
        math.sqrt((head[0] - apple_position[0])**2 + (head[1] + CELL_SIZE - apple_position[1])**2),
        math.sqrt((head[0] - apple_position[0])**2 + (head[1] - CELL_SIZE - apple_position[1])**2)
    ]

    # Choisir la direction qui minimise la distance
    best_direction = distances.index(min(distances))

    # Retourner la direction sous forme de tuple
    return [(1, 0), (-1, 0), (0, 1), (0, -1)][best_direction]
...
def main():
    ...
    while True:
        ...
        best_direction = get_best_direction()

        # Changer la direction du serpent
        change_speed_direction(best_direction)
        ...
        # Vérifier si toutes les pommes ont été mangées
        if len(snake_segments) == GRID_SIZE ** 2:
            print("Toutes les pommes ont été mangées !")
            pygame.quit()
            sys.exit()
...
def update_snake():
    ...
    new_head = pygame.Rect(
        snake_segments[0].x + direction[0] * CELL_SIZE,
        snake_segments[0].y + direction[1] * CELL_SIZE,
        CELL_SIZE, CELL_SIZE
    )
